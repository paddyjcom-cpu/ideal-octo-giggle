...
const PLATFORM_COLORS: Record<string, string> = { eBay: '#0064d2', Depop: '#ff3d00', Vinted: '#00c1d4', Other: '#64748b' };
...
const DashboardView: React.FC<{ user: UserProfile; stats: Stats; products: Product[]; onRevenueClick: () => void; onProfitClick: () => void; onSoldClick: () => void; onActiveClick: () => void; }> = ({ user, stats, products, onRevenueClick, onProfitClick, onSoldClick, onActiveClick }) => {
  const chartData = useMemo(() => {
    ...
    return months.map((month, index) => {
      ...
      const revenue = monthSold.reduce((a, b) => a + (b.soldPrice || 0), 0);
      return { name: month, revenue, profit: revenue - monthSold.reduce((a, b) => a + b.cost, 0) };
    });
  }, [products]);
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 pb-20">
      ...
      <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100">
        <h3 className="font-black text-slate-800 mb-6">Growth Trends</h3>
...
const StatCard = ({ title, value, icon, bgColor, onClick }: any) => (
  <button onClick={onClick} className="bg-white p-6 rounded-[2rem] shadow-sm border border-slate-100 flex flex-col group text-left transition-all hover:shadow-xl hover:border-indigo-200">
